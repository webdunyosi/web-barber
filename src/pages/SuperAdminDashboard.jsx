...
import { 
  ...
  FaSignOutAlt, FaEye, FaEyeSlash, FaGlobe, FaArrowLeft
} from 'react-icons/fa';
...
const SuperAdminDashboard = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingBarber) {
        ...
        if (!updateData.password.trim()) delete updateData.password; // don't update if blank
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white flex flex-col">
      ...
      <header className="bg-zinc-900/90 border-b border-emerald-500/20 py-4 px-6 sticky top-0 z-40 backdrop-blur-md">
        <div className="container mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            <FaUserTie className="text-emerald-500" size={24} />
            <h1 className="text-xl font-black uppercase text-emerald-400">Web Barber SaaS</h1>
            <span className="bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 text-[10px] font-bold py-1 px-2.5 rounded-full uppercase tracking-wider">
              Super Admin
            </span>
          </div>
          <button 
            onClick={() => { logout(); navigate('/'); }}
            className="flex items-center gap-2 bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/20 text-xs font-bold py-2 px-4 rounded-xl cursor-pointer transition-colors active:scale-98"
          >
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6">
              {/* Stat 1 */}
              <div className="bg-zinc-900/50 border border-zinc-800 rounded-3xl p-5 backdrop-blur-sm flex items-center justify-between">
                <div className="space-y-1">
                  <span className="text-xs text-zinc-500 font-bold uppercase tracking-wider">Sartaroshlar</span>
                  <p className="text-2xl font-black">{stats.barbersCount}</p>
                </div>
              ...
              <div className="bg-zinc-900/50 border border-zinc-800 rounded-3xl p-5 backdrop-blur-sm flex items-center justify-between">
                <div className="space-y-1">
                  <span className="text-xs text-zinc-500 font-bold uppercase tracking-wider">Mijozlar</span>
                  <p className="text-2xl font-black">{stats.clientsCount}</p>
                </div>
                <div className="w-12 h-12 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-emerald-400 shrink-0">
                  <FaUsers size={20} />
                </div>
              ...
              <div className="bg-zinc-900/50 border border-zinc-800 rounded-3xl p-5 backdrop-blur-sm flex items-center justify-between">
                <div className="space-y-1">
                  <span className="text-xs text-zinc-500 font-bold uppercase tracking-wider">Buyurtmalar</span>
                  <p className="text-2xl font-black">{stats.appointmentsCount}</p>
                </div>
                <div className="w-12 h-12 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-emerald-400 shrink-0">
              ...
              <div className="bg-zinc-900/50 border border-zinc-800 rounded-3xl p-5 backdrop-blur-sm flex items-center justify-between">
                <div className="space-y-1">
                  <span className="text-xs text-zinc-500 font-bold uppercase tracking-wider">Karta Tushumi</span>
                  <p className="text-2xl font-black text-emerald-400">{(stats.revenue || 0).toLocaleString()} UZS</p>
                </div>
                <div className="w-12 h-12 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-emerald-400 shrink-0">
                  <FaCoins size={20} />
                </div>
            ...
            {/* Barbers Management Title and Add button */}
            <div className="flex items-center justify-between pt-4">
              <div>
                <h2 className="text-2xl font-black tracking-wide">Sartaroshlar Ro'yxati</h2>
                <p className="text-xs text-zinc-500">Tizimga yangi sartaroshlarni qo'shishingiz va ularning mijoz havolalarini boshqarishingiz mumkin</p>
              </div>
              <button 
                onClick={handleOpenAddModal}
                className="flex items-center gap-1.5 bg-gradient-to-r from-emerald-500 to-green-600 text-white font-bold py-2.5 px-4 rounded-xl shadow-lg shadow-emerald-500/20 hover:scale-[1.02] active:scale-[0.98] transition-all cursor-pointer text-xs"
              >
                <FaPlus size={12} />
            ...
            {barbers.length === 0 ? (
              <div className="text-center py-16 bg-zinc-900/20 border border-zinc-850 rounded-3xl">
                ...
                <p className="text-zinc-650 text-xs mt-1">Yangi sartarosh qo'shish tugmasini bosib birinchisini qo'shing.</p>
              </div>
            ) : (
