...
const LoyaltyPage = () => {
  ...
  const stampsCount = isAuthenticated ? (user?.loyaltyStamps || 0) : 1;
      ...
      <div className="max-w-3xl mx-auto space-y-4 md:space-y-8">
        ...
        <div className="space-y-4 md:space-y-8 animate-fadeIn">
          ...
          <div className="bg-zinc-900/60 border border-white/10 rounded-3xl px-2 py-4 sm:p-6 backdrop-blur-xl shadow-xl flex flex-col items-center">
            ...
            {!isAuthenticated && (
              <div className="mt-6 w-full max-w-sm border-t border-white/5 pt-5 text-center space-y-3 animate-pulse">
                <p className="text-xs text-zinc-400">
                  Sadoqat kartangizni faollashtirish va ballar yig'ishni boshlash uchun profilingizga kiring.
                </p>
                <button
                  onClick={() => setIsAuthModalOpen(true)}
                  className="w-full bg-linear-to-br from-emerald-500 to-green-600 hover:shadow-lg hover:shadow-emerald-500/30 text-white font-bold py-3 px-4 rounded-xl transition-all active:scale-[0.98] border border-emerald-400 flex items-center justify-center gap-2 cursor-pointer text-sm shadow-md"
                >
                  <FaSignInAlt size={16} />
