...
const AdminPage = () => {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-zinc-950 text-white">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-lg font-semibold animate-pulse">Yuklanmoqda...</span>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-zinc-950 p-4">
        <div className="max-w-md w-full text-center bg-zinc-900 border border-red-500/30 rounded-2xl p-8 backdrop-blur-md shadow-2xl">
          ...
          <p className="text-gray-400 mb-6">Ushbu sahifa faqatgina sartarosh/administratorlar uchun mo'ljallangan.</p>
          <Link
            to="/"
            className="inline-flex items-center gap-2 bg-emerald-500 text-white font-semibold px-6 py-3 rounded-xl hover:bg-emerald-600 transition-all active:scale-95 shadow-lg shadow-emerald-500/20"
          >
  ...
  return (
    <div className="w-full lg:w-5/6 ml-auto pb-16 pt-4 text-white">
      {/* Title Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-white/10 pb-6 mb-8 px-4">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">Foydalanuvchilar, buyurtmalar va kunlik tushumlarni boshqarish</p>
        </div>
        ...
        <button
          ...
          className="bg-zinc-800 border border-zinc-700 hover:border-emerald-500/50 hover:bg-zinc-800/80 text-white font-semibold py-2.5 px-4 rounded-xl transition-all flex items-center justify-center gap-2 text-sm active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {isDataLoading ? 'Yangilanmoqda...' : 'Ma\'lumotlarni yangilash'}
        </button>
      </div>

      {/* Tabs Navigator */}
      <div className="flex border-b border-zinc-800 mb-8 overflow-x-auto gap-2 px-4 scrollbar-none">
        <button
          onClick={() => { setActiveTab('bookings'); setSearchTerm(''); }}
          className={`flex items-center gap-2 px-6 py-3.5 font-bold whitespace-nowrap transition-all rounded-t-xl text-base relative ${
            activeTab === 'bookings'
              ? 'text-emerald-400 bg-zinc-800/40 border border-b-0 border-zinc-800'
            ...
            <div className="space-y-6">
              ...
              <div className="flex flex-col md:flex-row gap-4 bg-zinc-900/50 border border-zinc-800 rounded-2xl p-4 backdrop-blur-sm">
                ...
                <div className="flex-1 relative">
                  ...
                  <input
                    ...
                    className="w-full pl-10 pr-4 py-2.5 bg-zinc-800 border border-zinc-700 rounded-xl outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent text-sm"
                  />
                </div>
                {/* Status select */}
              ...
              {filteredBookings.length === 0 ? (
                <div className="text-center py-12 bg-zinc-900/20 border border-dashed border-zinc-800 rounded-2xl">
                  ...
                  <p className="text-gray-400">Hech qanday buyurtma topilmadi.</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 gap-4">
                  {filteredBookings.map((booking) => (
                    <div
                      key={booking.id}
                      className={`relative bg-zinc-900/70 border rounded-2xl p-6 transition-all duration-300 backdrop-blur-sm flex flex-col md:flex-row justify-between gap-6 hover:border-zinc-700 ${
                        ...
                        {booking.status === 'pending' ? (
                          <div className="flex gap-2 w-full">
                            <button
                              ...
                              className="flex-1 bg-emerald-500 hover:bg-emerald-600 text-white font-bold py-2 px-3 rounded-xl text-xs transition-all active:scale-[0.97] flex items-center justify-center gap-1 disabled:opacity-50 cursor-pointer"
                            >
                            ...
                            <button
                              ...
                              className="flex-1 bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-3 rounded-xl text-xs transition-all active:scale-[0.97] flex items-center justify-center gap-1 disabled:opacity-50 cursor-pointer"
                            >
                          ...
                          <button
                            ...
                            className="w-full text-center text-xs text-white/40 hover:text-white hover:underline transition-colors disabled:opacity-50"
                          >
            ...
            <div className="space-y-6">
              ...
              {filteredUsers.length === 0 ? (
                <div className="text-center py-12 bg-zinc-900/20 border border-dashed border-zinc-800 rounded-2xl">
                  ...
                  <p className="text-gray-400">Hech qanday mijoz topilmadi.</p>
                </div>
              ) : (
                          ...
                          <td className="p-4 pr-6 text-right">
                            <div className="flex items-center justify-end gap-2">
                              ...
                              <button
                                ...
                                onClick={() => handleBlockUser(client.id, client.status)}
          ...
          {activeTab === 'statistics' && stats && (
            <div className="space-y-8 animate-fadeIn">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
                {/* Daily income */}
                <div className="bg-linear-to-br from-zinc-800/80 to-zinc-900/80 border border-emerald-500/30 rounded-2xl p-6 backdrop-blur-sm shadow-xl flex items-center justify-between">
                  <div className="space-y-1">
                    ...
                    <p className="text-xxs text-zinc-500">Oxirgi 24 soatlik tasdiqlanganlar</p>
                  </div>
                ...
                <div className="bg-linear-to-br from-zinc-800/80 to-zinc-900/80 border border-emerald-500/30 rounded-2xl p-6 backdrop-blur-sm shadow-xl flex items-center justify-between">
                  <div className="space-y-1">
                    ...
                    <p className="text-xxs text-zinc-500 font-medium text-emerald-400">Haftalik jami tushum</p>
                  </div>
                  <div className="w-12 h-12 rounded-xl bg-emerald-500/10 border border-emerald-500/20 flex items-center justify-center text-emerald-400">
                    <FaChartBar size={20} />
                  </div>
                ...
                {/* Users Count */}
                <div className="bg-linear-to-br from-zinc-800/80 to-zinc-900/80 border border-zinc-800 rounded-2xl p-6 backdrop-blur-sm shadow-xl flex items-center justify-between">
                  <div className="space-y-1">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 bg-zinc-900/50 border border-zinc-800 rounded-2xl p-6 backdrop-blur-sm">
                      ...
                      <polygon
                        points={`
                          ...
                          ${stats.chartData.map((d, index) => {
                            ...
                            // Calculate max point to scale Y to fit nicely (avoid division by 0)
                  ...
                  <div className="mt-4 border-t border-white/5 pt-4 flex justify-between items-center text-xs text-zinc-400">
                    <span>Maksimal kunlik: <strong className="text-white">{(Math.max(...stats.chartData.map(c => c.value))).toLocaleString()} so'm</strong></span>
                        ...
                        return (
                          <div key={service.name} className="space-y-1.5">
                            ...
                            <div className="flex justify-between text-xxs text-zinc-500">
                              ...
                              <span className="text-emerald-400/80 font-bold">{(service.revenue).toLocaleString()} so'm</span>
                            </div>
                          </div>
                        );
                      })}
